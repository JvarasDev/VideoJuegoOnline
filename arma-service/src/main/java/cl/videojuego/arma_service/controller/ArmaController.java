...
public class ArmaController {
        ...
        @Operation(summary = "Listar todas las armas", description = "Retorna una lista completa con todas las armas registradas en el sistema.")
        @ApiResponses({
        ...
        @Operation(summary = "Registrar nueva arma", description = "Permite registrar una nueva arma indicando nombre, daño, nivel mínimo, precio, tipo de arma y rareza.")
        @ApiResponses({
        ...
        @Operation(summary = "Listar armas por tipo", description = "Retorna las armas asociadas a un tipo específico, por ejemplo espada, arco o bastón.")
        @ApiResponses({
        ...
        public ResponseEntity<List<ArmaDTO>> listarPorTipo(
                        @Parameter(description = "Identificador del tipo de arma", example = "1") @PathVariable Long idTipoArma) {
                return ResponseEntity.ok(armaService.listarPorTipo(idTipoArma));
        }

        // GET: Buscar armas por rareza
        @Operation(summary = "Listar armas por rareza", description = "Retorna las armas asociadas a una rareza específica, por ejemplo común, rara, épica o legendaria.")
        @ApiResponses({
        ...
        public ResponseEntity<List<ArmaDTO>> listarPorRareza(
                        @Parameter(description = "Identificador de la rareza del arma", example = "2") @PathVariable Long idRarezaArma) {
                return ResponseEntity.ok(armaService.listarPorRareza(idRarezaArma));
        }

        // GET: Buscar armas por nivel mínimo exacto
        @Operation(summary = "Listar armas por nivel mínimo", description = "Retorna las armas que requieren un nivel mínimo específico para poder ser utilizadas.")
        @ApiResponses({
        ...
        public ResponseEntity<List<ArmaDTO>> listarPorNivel(
                        @Parameter(description = "Nivel mínimo requerido para utilizar el arma", example = "10") @PathVariable Integer nivelMinimo) {
                return ResponseEntity.ok(armaService.listarPorNivel(nivelMinimo));
        }

        // GET: Buscar armas por nombre parecido
        @Operation(summary = "Buscar armas por nombre", description = "Permite buscar armas utilizando una parte del nombre, sin distinguir mayúsculas o minúsculas.")
        @ApiResponses({
        ...
        public ResponseEntity<List<ArmaDTO>> buscarPorNombre(
                        @Parameter(description = "Nombre o parte del nombre del arma", example = "Espada") @RequestParam String nombre) {
                return ResponseEntity.ok(armaService.buscarPorNombre(nombre));
        }

        // GET: buscar armas con precio menor o igual

        @Operation(summary = "Buscar armas por precio máximo", description = "Retorna las armas cuyo precio sea menor o igual al valor indicado.")
        @ApiResponses({
        ...
        public ResponseEntity<List<ArmaDTO>> buscarPorPrecio(
                        @Parameter(description = "Precio máximo permitido para la búsqueda", example = "10000") @RequestParam Integer precio) {
                return ResponseEntity.ok(armaService.buscarPorPrecio(precio));
        }

        // GET: buscar arma por ID

        @Operation(summary = "Buscar arma por ID", description = "Permite obtener la información detallada de un arma específica mediante su identificador.")
        @ApiResponses({
        ...
        public ResponseEntity<ArmaDTO> buscarPorId(
                        @Parameter(description = "Identificador único del arma", example = "1") @PathVariable Long idArma) {
